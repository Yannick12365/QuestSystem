...
public class NPCQuestMenu extends CustomMenu implements Closeable, SlotCondition {
    ...
    public NPCQuestMenu(int size, NPC npc) {
        super(size);
        this.npc = npc;
    ...
    public InventoryContent getContents(Player player) {
        ...
        content.fill(11, 16, new InventoryItem(new ItemManager(Material.BOOK).build(), ()->{
            ...
            InventoryMenuManager.getInstance().openMenu(player, new ConfirmQuestMenu(54, new QuestManager().getNPCQuests(npc.getName()).get(1)));
